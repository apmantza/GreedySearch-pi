...
- **jsdom/Readability/Turndown lazy-loaded** (`src/fetcher.mjs`) — The heavy extraction dependencies now load on first use instead of at process startup, cutting ~150-400ms and 30-60MB from every search child that never fetches a source; jsdom windows are now closed in a `finally`. (#34)
- **Fixed sleeps replaced with condition polls** (`bin/cdp.mjs`, `src/search/fetch-source.mjs`, `src/search/chrome.mjs`) — Tab-daemon connect tries immediately with backoff instead of a flat 300ms pre-sleep; `cdp list` exits without its 100ms timer; `navStr` skips the redundant readyState poll when the load event fired and invalidates the cached execution context once per navigation; post-nav 800ms sleeps became readyState/content polls (200ms interval, 4s cap); Chrome mode-switch and launch-lock waits became deadline polls (the old flat 3s lock wait could spawn a second contending Chrome). (#35)
- **Extractor polling converted to in-browser promises** (`extractors/common.mjs`, `perplexity.mjs`, `chatgpt.mjs`, `gemini.mjs`, `bing-copilot.mjs`, `consensus.mjs`) — New shared `clickCopyAndAwaitClipboard` replaces four copy-pasted click/fixed-sleep/read-clipboard flows (resolves the moment the clipboard sentinel lands instead of sleeping 400-2600ms); `waitForCopyButton` polls in-page; Gemini skips re-polling a predicate the stream-wait already guaranteed; Consensus `expandReferences` runs as one in-browser loop keyed on card-count growth instead of two evals + 1500ms sleep per Load-More click. (#36)
- **Final-round quality evaluation skipped** (`src/search/research.mjs`) — The last research round no longer spends a Gemini browser round-trip (~20-60s) computing next-actions the loop can't use; the score falls back to the latest quality history entry. (#38, part 1 — the evidence+learning prompt merge remains open)
...
- **Gemini evidence/learning extraction no longer fails on long research prompts** (`src/search/research.mjs`) — Gemini's composer hard-caps input at ~32k chars; the merged evidence+learning prompt reached ~54k and was silently truncated, failing extraction on every research round. The builder now uses tighter per-source budgets (Task A 4×3000, Task B 6×2000), compact JSON for embedded context, and a `MAX_PROMPT_CHARS` (28000) guard that deterministically shrinks the assembled prompt to fit. Verified live: evidence and learnings now populate across rounds. (#52)
- **Input-text verification tolerates whitespace-collapsing editors** (`extractors/common.mjs`, `gemini.mjs`, `consensus.mjs`, `logically.mjs`) — Defensive hardening: the insertion check compared the editor's whitespace-normalized `innerText` against the raw prompt length. New shared `verifyInputText` normalizes both sides and matches head/tail slices, and all three extractors now clear and retype once before failing. (#52)
...
- **jitter() matches its documented ±20%** (`extractors/common.mjs`) — Node-side variance was ±40% while in-browser copies used ±20%; aligned to ±20%. (#36)
