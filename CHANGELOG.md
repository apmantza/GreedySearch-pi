...
- **Input-text verification tolerates whitespace-collapsing editors** (`extractors/common.mjs`, `gemini.mjs`, `consensus.mjs`, `logically.mjs`) — Long research prompts (notably the merged evidence+learning prompt) falsely failed with "input field did not accept text" because the check compared the editor's whitespace-normalized `innerText` against the raw prompt length. New shared `verifyInputText` normalizes both sides and matches head/tail slices, and all three extractors now clear and retype once before failing. (#52)
