...
- **Consensus extractor** (`extractors/consensus.mjs`, `extractors/common.mjs`, `bin/cdp.mjs`, `src/search/constants.mjs`, `src/search/recovery.mjs`) — New engine for `consensus.app` that submits a research query, waits for the prose answer to stream in, clicks "Load more results" to expand the references list, then intercepts the `Export → .CSV` menu's underlying `/api/papers/details/` POST response for rich source metadata (title, authors, year, journal, DOI, citation count, abstract takeaway, badges). Falls back to DOM `a[data-testid="search-result"]` cards when the API interception yields no sources. Detects the `/sign-up/` wall (URL-based, language-agnostic) and surfaces a clear "please sign in" error matching the Bing/Perplexity pattern. Registered as `consensus` / `cns` in `ENGINES` and `consensus` in `ENGINE_DOMAINS`, added to `HEADLESS_RECOVERY_ENGINES` so the orchestrator auto-recovers to visible mode on signin blocks. All selectors are language-agnostic (textarea `name="newMessage"`, button `aria-label="Submit search"`, Tailwind `prose` container, `/papers/` URL fragment, `.CSV` developer-set constant). **Not in default `ALL_ENGINES`** — opt-in via `~/.pi/greedyconfig` `engines` array.
...
- **Consensus stale Clerk session recovery** (`extractors/consensus.mjs`) — Detects the broken Clerk handshake state (`clerk.consensus.app`, `session-token-expired`, `refresh_request_origin_azp_mismatch`, or Chrome's `HTTP ERROR 405` error page), clears only `https://consensus.app` and `https://clerk.consensus.app` storage, then retries navigation once. If the retry still lands in the stale auth state, the extractor marks `blockedBy: "signin"` / `verificationResult: "needs-human"` and leaves visible Chrome open for the user to sign in again. This avoids periodically wiping the entire GreedySearch profile while recovering from corrupted Consensus auth cookies.
