...
async function netStr(cdp, sid) {
	...
	try {
		...
		throw new Error(`Invalid network entries JSON: ${error.message}`);
	}
	return entries
		.map(
			(e) =>
...
async function clickStr(cdp, sid, selector) {
  ...
  `;
	...
	try {
		...
		throw new Error(`Invalid click result JSON: ${error.message}`);
	}
...
async function getOrStartTabDaemon(targetId) {
	...
	const daemonScript = fileURLToPath(
		new URL("../../bin/cdp.mjs", import.meta.url),
	);
	const child = spawn(
		process.execPath,
		[daemonScript, "_daemon", targetId],
		{
			detached: true,
			stdio: "ignore",
		},
...
export function sendCommand(conn, req, timeoutMs = 0) {
	return new Promise((resolve, reject) => {
		let buf = "";
		let settled = false;
		let timer;
		...
		if (timeoutMs > 0) {
			timer = setTimeout(() => {
				...
				cleanup();
				conn.destroy();
				reject(new Error(`cdp timeout: ${req.cmd}`));
			}, timeoutMs);
		}
		req.id = 1;
...
export const USAGE = `cdp - lightweight Chrome DevTools Protocol CLI (no Puppeteer)
...
/** Error carrying the CLI's exact stderr/stdout for the thin CLI wrapper. */
...
function validateArgv(argv) {
	...
	if (!VALID_COMMANDS.has(argv[0])) {
		throw new CdpCommandError(`Unknown command: ${argv[0]}`, {
...
function readPagesCache() {
	try {
		...
		throw new Error(`Invalid page list cache: ${error.message}`);
	}
}

async function listPages(timeoutMs) {
	let pages;
	const existingSock = findAnyDaemonSocket();
	if (existingSock) {
		try {
			const conn = await connectToSocket(existingSock);
			const resp = await sendCommand(conn, { cmd: "list_raw" }, timeoutMs);
			if (resp.ok) pages = JSON.parse(resp.result);
		} catch {}
	}
	if (!pages) {
		const browserCdp = new CDP();
		try {
			await browserCdp.connect(getWsUrl());
			pages = await getPages(browserCdp);
		} finally {
			browserCdp.close();
		}
	}
...
function resolveTarget(targetPrefix) {
	if (!targetPrefix) {
		throw new CdpCommandError('Error: target ID required. Run "cdp list" first.');
	}
	...
	if (daemonMatches.length > 0) {
		return resolvePrefix(targetPrefix, daemonTargetIds, "daemon");
	}
	if (!existsSync(PAGES_CACHE)) {
		throw new CdpCommandError('No page list cached. Run "cdp list" first.');
	}
...
function normalizeCommandArgs(cmd, args) {
	const cmdArgs = args.slice();
	if (cmd === "eval") {
		const expr = cmdArgs.join(" ");
		if (!expr) throw new CdpCommandError("Error: expression required");
		cmdArgs[0] = expr;
	} else if (cmd === "type") {
		const text = cmdArgs.join(" ");
		if (!text || cmdArgs[0] === "--stdin")
			throw new CdpCommandError("Error: text required");
		cmdArgs[0] = text;
	} else if (cmd === "evalraw") {
		if (!cmdArgs[0]) throw new CdpCommandError("Error: CDP method required");
		if (cmdArgs.length > 2) cmdArgs[1] = cmdArgs.slice(1).join(" ");
	} else if (cmd === "browse" || cmd === "browserraw") {
		if (!cmdArgs[0])
			throw new CdpCommandError("Error: browser CDP method required");
		if (cmdArgs.length > 2) cmdArgs[1] = cmdArgs.slice(1).join(" ");
	}
	if ((cmd === "nav" || cmd === "navigate") && !cmdArgs[0]) {
		throw new CdpCommandError("Error: URL required");
	}
	return cmdArgs;
}

/**
 * Execute one command using the per-tab daemon client.
 *
 * This has the same argv contract as bin/cdp.mjs, but returns the command
 * result directly instead of starting a Node process and printing it.
 */
export async function cdpCommand(argv, timeoutMs = 0) {
	validateArgv(argv);
	const [cmd, ...args] = argv;

	if (!cmd || cmd === "help" || cmd === "--help" || cmd === "-h")
		return USAGE;
	if (cmd === "list" || cmd === "ls") return listPages(timeoutMs);
	if (cmd === "stop") {
	...
	if (!NEEDS_TARGET.has(cmd)) {
		throw new CdpCommandError(`Unknown command: ${cmd}`, {
			stderr: `Unknown command: ${cmd}\n\n`,
			stdout: USAGE,
		});
	}

	const targetId = resolveTarget(args[0]);
	const cmdArgs = normalizeCommandArgs(cmd, args.slice(1));
	const conn = await getOrStartTabDaemon(targetId);
	let response;
	try {
		response = await sendCommand(conn, { cmd, args: cmdArgs }, timeoutMs);
	} finally {
		// The daemon protocol is one JSON request/response per connection.  The
		// normal response path ends the socket; destroy any exceptional path so
		// an in-process caller cannot retain a pipe or Unix socket indefinitely.
		if (!conn.destroyed && !conn.writableEnded) conn.destroy();
	}
	if (response.ok) return response.result ?? "";
	throw new CdpCommandError(`Error: ${response.error}`, {
		stderr: `Error: ${response.error}\n`,
	});
}

export { getOrStartTabDaemon, runDaemon };
