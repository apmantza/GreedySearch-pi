...
3. **Anonymous landing textarea IS a usable chat input** — chatgpt.com ships `<textarea name="prompt">` + `button[aria-label="Send message"]` on its anonymous `/uc/<uuid>` chat surface, which works without sign-in for a single-query sandbox. The old `PROSE_SELECTOR = "div.ProseMirror"` only matched signed-in `/c/<uuid>` conversations; if it didn't resolve, the extractor fell through to a body-text regex that incorrectly flagged the page as a login wall. The current code dispatches on whichever selector resolves first. The anonymous path uses a React-friendly native value setter (plain `.value = ...` is ignored by the controlled component, so the send button stays disabled) and `typeAndSubmitLanding` clicks the textarea's native `Send message` button — **not** `button[data-testid="send-button"]`, which only exists in the signed-in flow. The response lives between `"ChatGPT said:"` and the next `"You said:"` or known footer (`ChatGPT is AI and can make mistakes.`, `You'll get smarter responses…`, `Chat with ChatGPT`); `extractAnswerAnonymous` slices it out without relying on the signed-in `data-message-author-role` attributes. Do not regress to a body-text regex check for sign-in — the anonymous page renders "Log in" / "Sign up for free" in its nav even though the chat itself works without auth.
